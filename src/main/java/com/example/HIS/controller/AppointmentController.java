...
//        System.out.println(patient.getPatientIdentity());
    ...
    public void addRemainNumberDaily(){
        ...
        if (result.equals("08:00:00")){//每天8点更新
            for (int i=9;i<17;i++){
                Date day=DateTool.getFutureDate(1,i); //获取六天后各时间点6
                RemainDeptNumber remainDeptNumber = new RemainDeptNumber();
    ...
    public void addRemainNumber() throws ParseException {
        ...
        for (int day=1;day<13;day++){
            for (int i=9;i<17;i++){
                String day_string;
                if (i<10){
                    day_string="2023-01-"+String.valueOf(day)+" 0"+String.valueOf(i)+":00:00";
                }
                else{
                    day_string="2023-01-"+String.valueOf(day)+" "+String.valueOf(i)+":00:00";
                }
